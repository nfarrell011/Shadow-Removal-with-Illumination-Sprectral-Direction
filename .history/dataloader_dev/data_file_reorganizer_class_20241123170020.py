...
class DataFileReorganizer:
    ...
    def reorganize_data_files(self, parent_data_folder_name:str, images_folder_list:list, image_quality_list:list) -> None:
        """ 
        ...
        # Root
        training_data_root = str(cwd / "training_data")
        os.makedirs(training_data_root, exist_ok = True)
        
        # Img destination
        training_images_folder = str(cwd / "training_data" / "training_images")
        os.makedirs(training_images_folder, exist_ok = True)
        
        # Img destination
        training_isd_maps_folder = str(cwd / "training_data" / "training_isds")
        ...
        for img_folder in images_folder_list:
            ...
            for image_quality in image_quality_list:
                    ...
                    # Set folder paths
                    image_folder = cwd / parent_data_folder_name / img_folder / f"processed" / image_quality
                    isd_map_folder = cwd / parent_data_folder_name / img_folder / f"processed" / "isd_maps"
    ...
    def get_min_max_dims(self, folder_name:str) -> tuple:
        """ 
    ...
    def center_crop_and_split(self, folder_name: str, output_folder: str, crop_size:int, sub_crop_size:int):
        """
        Center crop all images and then divide the center crop into four 224x224 sub-crops.
        """
        # Pathing
        cwd = Path.cwd()
        image_folder = str(cwd / "training_data" / folder_name)
        output_dir = Path(cwd / "training_data" / output_folder)
